...
const APPROVAL_CONTACTS = [
    ...
    { key: `ziad_hourany`, name: `زياد`, role: `قسم الطلبيات`, phone: `0775040502`, email: `Ziad.hourany@dadgroup.com` }
];
...
function pendingOwner(order = {}) {
    ...
    if ([`finance_approved`, `orders_staff_pending`].includes(status)) return `orders_staff`;
    return ``;
}
...
function buildReminderMessage(contact, orders) {
    ...
    const action = contact.key === `ziad_hourany` ? `معالجة` : `الموافقة على`;
    const actionCount = contact.key === `ziad_hourany` ? `معالجتها` : `الموافقة عليها`;
    const details = orders.map((order, index) => `${index + 1}- ${pharmacyName(order)} وقيمتها ${formatMoney(orderValue(order))} د.ا`).join(`\n`);
    return `مرحباً ${contact.name}،\nلديك طلبيات عدد ${orders.length} يرجى ${action} الطلبيات.\nإليك تفصيل الطلبيات:\n${details}\n\nعدد الطلبيات المطلوب منك ${actionCount} هو: ${orders.length}\nقيمة الطلبيات المطلوبة هي: ${formatMoney(total)} د.ا`;
}
...
function reminderCardHtml(contact, orders) {
    ...
    return `<article class="approval-reminder-card" data-reminder-contact="${contact.key}">
        ...
        <div class="approval-reminder-stats">
            <div class="approval-reminder-stat"><span>بانتظار الإجراء</span><strong>${orders.length} طلبية</strong></div>
...
async function loadApprovalReminders() {
    ...
    try {
        ...
        const orders = await fetchPendingApprovalOrders();
        ...
        orders.forEach(order => {
            ...
            if (owner === `orders_staff`) {
                // A completed export or invoice no longer needs Ziad's action.
                if (order.orderStaffStatus === `orders_staff_exported` || order.orderStaffStatus === `orders_staff_hidden` || order.exportedAt || order.invoicedAt) return;
        ...
        grid.querySelectorAll(`[data-reminder-action]`).forEach(button => {
            button.addEventListener(`click`, () => {
                ...
                const subject = `تذكير بمتابعة الطلبيات (${ordersForContact.length})`;
...
$rem(`managerReminderBtn`)?.addEventListener(`click`, async () => {
    ...
    sections.forEach(id => { const section = $rem(id); if (section) section.style.display = id === `supervisorReminderSection` ? `block` : `none`; });
    [$rem(`advancedManagerDashboard`), $rem(`managerDateFilters`)].forEach(element => { if (element) element.style.display = `none`; });
    document.querySelectorAll(`.supervisor-premium-tabs .btn-subtab[data-target-section]`).forEach(button => button.classList.toggle(`active`, button.id === `managerReminderBtn`));
