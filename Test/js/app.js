...
import { loadSystemConfig, verifyPassword, routeForRole, normalizeRole, guardPage, applySystemPresentation } from './system-config.js?v=20260712_settings_control_center_v2';
...
function initializeManagerView(managerName) {
    ...
    if (managerAddBtn) {
        managerAddBtn.hidden = !appCan('order.create_for_rep');
        managerAddBtn.onclick = () => {
            if (!requireAppPermission('order.create_for_rep')) return;
...
function applyRememberPasswordPolicy() {
    const allowed = runtimeSystemConfig?.general?.allowRememberPasswords !== false;
    ['rememberRepPass', 'rememberAdmin', 'rememberMe'].forEach(id => {
        const input = getEl(id);
        if (!input) return;
        input.checked = allowed && input.checked;
        input.disabled = !allowed;
        const wrapper = input.closest('.remember-container, .input-group') || input.parentElement;
        if (wrapper) wrapper.hidden = !allowed;
    });
    if (!allowed) {
        Object.keys(localStorage).filter(key => key.startsWith('savedRepPass_')).forEach(key => localStorage.removeItem(key));
    }
}

function appCan(permission) {
    const session = getAdminSession();
    const role = normalizeRole(session?.role || session?.type || '');
    const user = role
        ? (runtimeSystemConfig?.users || []).find(item => item.id === session?.userId) ||
          (runtimeSystemConfig?.users || []).find(item => item.displayName === session?.name && normalizeRole(item.role) === role)
        : null;
    if (user?.active === false) return false;
    if (!runtimeSystemConfig?.general?.enforcePermissions) return true;
    if (!role || !user) return false;
    if (Object.prototype.hasOwnProperty.call(user.permissions || {}, permission)) return user.permissions[permission] === true;
    return runtimeSystemConfig.rolePermissions?.[role]?.[permission] === true;
}

function requireAppPermission(permission) {
    if (appCan(permission)) return true;
...
async function confirmSupervisorDeleteOrder(orderId, order = {}, action = 'supervisor_order_soft_deleted') {
    if (!requireAppPermission('order.delete_before_approval')) return;
...
async function approveOrderBySupervisor(orderId, order = {}, action = 'supervisor_approved') {
    if (!requireAppPermission('order.approve_supervisor')) return;
...
async function supervisorReturnToRep(orderId, order = {}) {
    if (!requireAppPermission('order.return_to_rep')) return;
...
function saveAdminSession(name, type, remember, details = {}) {
    const normalizedRole = normalizeRole(details.role || type || '');
    const session = {
        name,
        type: normalizedRole === 'supervisor' ? 'manager' : normalizedRole,
        role: normalizedRole,
...
async function loadInitialData() {
    try {
        if (APP_PAGE === 'login') normalizeLoginUrlAfterSwitch();
        runtimeSystemConfig = await loadSystemConfig(true);
        applySystemPresentation(runtimeSystemConfig);
        applyRememberPasswordPolicy();
        if (APP_PAGE === 'supervisor' && !(await guardPage('page.supervisor'))) return;
        systemAdminUsers = (runtimeSystemConfig.users || []).filter(user => user.active !== false);
        if (APP_PAGE === 'login') initializeAdminAccounts(systemAdminUsers);
        if (repSelect) {
            repSelect.innerHTML = '<option value="">⏳ جاري تحميل البيانات...</option>';
            repSelect.disabled = true;
        }

        const cacheVersion = runtimeSystemConfig?.general?.cacheVersion || '20260625_report_logic_cache_fix1';
        const dataRevision = runtimeSystemConfig?.general?.dataRevision || 1;
        const CACHE_KEY = `dad_app_cache_${cacheVersion}_${dataRevision}`;
        const CACHE_TIME_KEY = `dad_app_cache_time_${cacheVersion}_${dataRevision}`;
        ...
        repDirectoryById = new Map();
        repsData.forEach(rep => {
            const repId = String(rep.id || '').trim();
            const repName = String(rep.name || repId).trim();
            if (!repId || !repName) return;
            repDirectoryById.set(repId, rep);
            // اكتب القيمة دائماً؛ القيمة الفارغة تعني إزالة المشرف القديم فعلياً.
            repManagerMap[repName] = String(rep.supervisorName || rep.managerName || rep.supervisor || '').trim();
            if (rep.passwordHash) repPasswordsMap[repName] = String(rep.passwordHash).trim();
        });

        if (repSelect) {
            repSelect.innerHTML = '<option value="">-- اختر المندوب --</option>';
            repsData.filter(d => d.active !== false).forEach(d => {
                const opt = document.createElement('option');
                opt.value = d.id;
...
async function bootstrapPage() {
    ...
    if (APP_PAGE === 'supervisor') {
        const adminSession = getAdminSession();
        if (!adminSession || normalizeRole(adminSession.role || adminSession.type) !== 'supervisor') return goToLogin();
        if (!(await guardPage('page.supervisor'))) return;
        isAdmin = true;
        const currentUser = (runtimeSystemConfig?.users || []).find(user => user.id === adminSession.userId) ||
            (runtimeSystemConfig?.users || []).find(user => user.displayName === adminSession.name && normalizeRole(user.role) === 'supervisor');
        currentManagerName = currentUser?.displayName || adminSession.name;
        initializeManagerView(currentManagerName);
    }
    ...
    if (APP_PAGE === 'login') {
        ...
        if (adminSession && adminSession.remember && !isAdminOrderMode) {
            window.location.href = routeForRole(adminSession.role || adminSession.type, adminSession.route || '');
        }
    }
}
...
if (repSelect) repSelect.onchange = async (e) => {
    ...
    // استرجاع كلمة المرور فقط عندما يسمح إعداد النظام بالحفظ المحلي.
    const rememberAllowed = runtimeSystemConfig?.general?.allowRememberPasswords !== false;
    const savedPass = rememberAllowed ? localStorage.getItem('savedRepPass_' + e.target.value) : '';
    if (savedPass) {
...
if (startOrderBtn) startOrderBtn.onclick = async (e) => { e.preventDefault(); // 🟢 أضف هذا السطر لمنع إرسال الفورم...
    ...
    const repRecord = repDirectoryById.get(repSelect.value) || {};
    const expectedHash = repRecord.passwordHash || repPasswordsMap[selectedRepNameText] || '';
    const passwordRequired = runtimeSystemConfig?.general?.representativePasswordRequired !== false;

    if (passwordRequired && !expectedHash) {
        repPassInput.classList.add('input-error');
        return showToast("لم يتم تعيين كلمة سر لهذا المندوب. راجع مركز الإعدادات.", "error");
    }

    if (passwordRequired && !enteredPass) {
        repPassInput.classList.add('input-error');
        return showToast("الرجاء إدخال الرقم السري الخاص بك.", "warning");
    }

    if (passwordRequired && !(await verifyPassword(enteredPass, expectedHash))) {
        repPassInput.classList.add('input-error');
    ...
    if (runtimeSystemConfig?.general?.allowRememberPasswords !== false && document.getElementById('rememberRepPass')?.checked) {
    ...
    const isAdminOrder = sessionStorage.getItem('adminOrderMode') === '1' && normalizeRole(adminOrderSession?.role || adminOrderSession?.type) === 'supervisor';
...
async function openEditOrder(orderId, userType) {
    ...
    if (isSupervisorOrderEdit && !requireAppPermission('order.edit_supervisor')) return;
...
function adminRoleIcon(role = '') {
    return ({
        supervisor: 'ph-user-tie',
        market_manager: 'ph-briefcase',
        finance_controller: 'ph-bank',
        orders_staff: 'ph-package',
        reports: 'ph-chart-pie-slice',
        settings: 'ph-gear-six',
        audit: 'ph-shield-check',
        control: 'ph-command'
    })[normalizeRole(role)] || 'ph-user-circle-gear';
}

function bindAdminOptionButtons() {
    document.querySelectorAll('.btn-admin-opt').forEach(btn => {
        btn.onclick = (e) => {
            ...
            selectedAdminType = normalizeRole(targetBtn.getAttribute('data-type') || '');
            selectedAdminName = targetBtn.getAttribute('data-name') || '';
            selectedAdminUser = systemAdminUsers.find(user => user.id === targetBtn.dataset.userId) || {
                id: targetBtn.dataset.userId || '',
                displayName: selectedAdminName,
                role: selectedAdminType,
                passwordHash: targetBtn.dataset.passwordHash || 'MjAyNjA0',
                route: targetBtn.dataset.route || '',
                permissions: {}
            };
        };
    });
...
function initializeAdminAccounts(users = []) {
    const grid = document.querySelector('.admin-options-grid');
    if (!grid) return;
    const activeUsers = users.filter(user => user.active !== false);
    if (!activeUsers.length) return bindAdminOptionButtons();
    grid.innerHTML = activeUsers.map(user => `
        <button class="btn-admin-opt" data-user-id="${escapeHtml(user.id)}" data-name="${escapeHtml(user.displayName)}" data-type="${escapeHtml(normalizeRole(user.role))}" data-route="${escapeHtml(user.route || '')}" type="button">
            <i class="ph ${adminRoleIcon(user.role)}"></i>
            <span>${escapeHtml(user.displayName)}</span>
            <small>${escapeHtml((runtimeSystemConfig?.roleLabels || {})[user.role] || ({ supervisor: 'مشرف', market_manager: 'مدير السوق', finance_controller: 'المراقب المالي', orders_staff: 'قسم الطلبيات', reports: 'التقارير', settings: 'الإعدادات', audit: 'التدقيق', control: 'التحكم' })[normalizeRole(user.role)] || user.role)}</small>
        </button>`).join('');
    bindAdminOptionButtons();
}
...
if (confirmAdminLoginBtn) confirmAdminLoginBtn.onclick = async (e) => {
    e.preventDefault();
    if (!selectedAdminType || !selectedAdminUser) return showToast("الرجاء تحديد هويتك من البطاقات أعلاه", "warning");
    const pass = getEl('adminPasswordInput')?.value || '';
    const passwordOk = await verifyPassword(pass, selectedAdminUser.passwordHash || 'MjAyNjA0');
    if (!passwordOk) return showToast("رمز المرور غير صحيح!", "error");

    const rememberMe = runtimeSystemConfig?.general?.allowRememberPasswords !== false && !!(getEl('rememberAdmin')?.checked || getEl('rememberMe')?.checked);
    ...
    sessionStorage.removeItem('adminOrderMode');
    window.location.href = routeForRole(selectedAdminUser.role || selectedAdminType, selectedAdminUser.route || '');
};
