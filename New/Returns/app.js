...
const SAVED_REP_LOGIN = `returns_saved_rep_login`;
...
async function editRepReturn(row) {
    const pharmacy = ($(`pharmacySelect`)._rows || []).find(item => item.repPharmacyKey === row.repPharmacyKey);
    if (!pharmacy) return showBanner(`تعذر العثور على الصيدلية المرتبطة بهذا المرتجع.`, `error`);
    editingId = row.id; selectedPharmacy = pharmacy; $(`pharmacySelect`).value = pharmacy.id; $(`pharmacySelect`)._searchCombobox?.sync(); $(`pharmacyCode`).value = pharmacy.pharmacyCode; $(`pageIdentity`).textContent = `${repName} — ${pharmacy.pharmacyName}`; await loadPharmacySales(); $(`returnLines`).innerHTML = ``; row.items.forEach(addLine); $(`returnNote`).value = row.note || ``; switchRepTab(`new`); showBanner(`تم فتح المرتجع للتعديل وإعادة الإرسال.`, `info`);
}
...
async function initRep() {
    const access = readAccessSession(); if (access?.role !== `rep` || !access.repName) { location.replace(`login.html`); return; }
    repName = text(access.repName); $(`pageIdentity`).textContent = repName; $(`addLine`).disabled = true; $(`submitReturn`).disabled = true;
    await loadRepPharmacies(); enhanceCombobox($(`pharmacySelect`), { placeholder: `اكتب اسم الصيدلية أو الكود...` });
    const selectPharmacy = async pharmacyId => {
        selectedPharmacy = ($(`pharmacySelect`)._rows || []).find(row => row.id === pharmacyId) || null; editingId = ``; $(`returnLines`).innerHTML = ``; $(`returnNote`).value = ``; $(`pharmacyCode`).value = selectedPharmacy?.pharmacyCode || ``; $(`addLine`).disabled = !selectedPharmacy; $(`submitReturn`).disabled = !selectedPharmacy;
        if (!selectedPharmacy) { $(`pageIdentity`).textContent = repName; updateTotal(); return; }
        $(`pageIdentity`).textContent = `${repName} — ${selectedPharmacy.pharmacyName}`; writeAccessSession({ ...access, pharmacyId:selectedPharmacy.id, pharmacyCode:selectedPharmacy.pharmacyCode, pharmacyName:selectedPharmacy.pharmacyName, supervisorName:selectedPharmacy.supervisorName, createdAt:Date.now() }); await loadPharmacySales(); addLine();
    };
    $(`pharmacySelect`).onchange = event => selectPharmacy(event.target.value); $(`addLine`).onclick = () => addLine(); $(`submitReturn`).onclick = submitRepReturn; document.querySelectorAll(`[data-tab]`).forEach(button => button.onclick = () => switchRepTab(button.dataset.tab));
    const rememberedPharmacy = ($(`pharmacySelect`)._rows || []).find(row => row.id === access.pharmacyId || row.pharmacyCode === access.pharmacyCode); if (rememberedPharmacy) { $(`pharmacySelect`).value = rememberedPharmacy.id; $(`pharmacySelect`)._searchCombobox?.sync(); await selectPharmacy(rememberedPharmacy.id); }
}

let reviewRows = [];
function roleStatus() { return Object.keys(STATUS_LABELS); }
function renderReviewShell() {
    $(`reviewRoot`).innerHTML = `<section class="card"><div class="summary"><div class="stat"><span>عدد المرتجعات</span><strong id="countStat">0</strong></div><div class="stat"><span>إجمالي الكمية</span><strong id="qtyStat">0</strong></div><div class="stat"><span>الكمية المدفوعة</span><strong id="paidStat">0</strong></div><div class="stat"><span>القيمة</span><strong id="valueStat">0.00</strong></div></div><div class="grid smart-filter-grid"><div class="field wide"><label>بحث شامل</label><input id="searchFilter" placeholder="الصيدلية، المندوب، الفاتورة، الصنف أو Batch"></div><div class="field"><label>الحالة</label><select id="statusFilter"><option value="">جميع الحالات</option></select></div><div class="field"><label>المندوب</label><select id="repFilter"><option value="">جميع المناديب</option></select></div><div class="field"><label>الصيدلية</label><select id="pharmacyFilter"><option value="">جميع الصيدليات</option></select></div><div class="field"><label>الصنف</label><select id="productFilter"><option value="">جميع الأصناف</option></select></div><div class="field"><label>Batch</label><select id="batchFilter"><option value="">جميع الـBatches</option></select></div><div class="field"><label>من تاريخ</label><input id="fromFilter" type="date"></div><div class="field"><label>إلى تاريخ</label><input id="toFilter" type="date"></div><div class="field filter-reset-wrap"><label>&nbsp;</label><button id="resetFilters" class="btn secondary" type="button"><i class="ph ph-arrow-counter-clockwise"></i> مسح الفلاتر</button></div></div></section><section class="card"><div class="table-wrap"><table class="table"><thead><tr><th><input id="selectAll" class="check" type="checkbox"></th><th>التاريخ</th><th>المندوب / المشرف</th><th>الصيدلية</th><th>الأصناف</th><th>الكمية / البونص</th><th>القيمة</th><th>الحالة</th><th>الإجراء</th></tr></thead><tbody id="reviewBody"></tbody></table></div></section>`;
    [`searchFilter`,`fromFilter`,`toFilter`].forEach(id => $(id).oninput = renderReviewRows); [`statusFilter`,`repFilter`,`pharmacyFilter`,`productFilter`,`batchFilter`].forEach(id => { $(id).onchange = renderReviewRows; enhanceCombobox($(id), { placeholder: $(id).options[0].textContent }); }); $(`resetFilters`).onclick = () => { [`searchFilter`,`fromFilter`,`toFilter`].forEach(id => $(id).value = ``); [`statusFilter`,`repFilter`,`pharmacyFilter`,`productFilter`,`batchFilter`].forEach(id => { $(id).value = ``; $(id)._searchCombobox?.sync(); }); renderReviewRows(); }; $(`selectAll`).onchange = event => document.querySelectorAll(`.row-check`).forEach(box => box.checked = event.target.checked);
}
function filteredReviewRows(ignored = ``) {
    const needle = normalize($(`searchFilter`)?.value), status = text($(`statusFilter`)?.value), rep = text($(`repFilter`)?.value), pharmacy = text($(`pharmacyFilter`)?.value), product = text($(`productFilter`)?.value), batch = text($(`batchFilter`)?.value), from = text($(`fromFilter`)?.value), to = text($(`toFilter`)?.value);
    return reviewRows.filter(row => { const items = row.items || [], haystack = normalize(`${row.repName} ${row.supervisorName} ${row.pharmacyCode} ${row.pharmacyName} ${items.map(x => `${x.invoiceNumber} ${x.productCode} ${x.productName} ${x.batch}`).join(` `)}`), created = row.createdAt?.toDate ? row.createdAt.toDate().toISOString().slice(0,10) : isoDate(row.createdAt), needsItemFilter = (ignored !== `product` && product) || (ignored !== `batch` && batch), itemMatch = items.some(item => (ignored === `product` || !product || normalize(item.productCode || item.productName) === product) && (ignored === `batch` || !batch || normalize(item.batch) === batch)); return (!needle || haystack.includes(needle)) && (ignored === `status` || !status || row.status === status) && (ignored === `rep` || !rep || normalize(row.repName) === rep) && (ignored === `pharmacy` || !pharmacy || normalize(row.pharmacyCode) === pharmacy) && (!needsItemFilter || itemMatch) && (!from || created >= from) && (!to || created <= to); });
}
function smartFilterValues(filterName) {
    const rows = filteredReviewRows(filterName), values = new Map();
    rows.forEach(row => {
        if (filterName === `status`) values.set(row.status, STATUS_LABELS[row.status] || row.status);
        else if (filterName === `rep`) values.set(normalize(row.repName), row.repName);
        else if (filterName === `pharmacy`) values.set(normalize(row.pharmacyCode), `${row.pharmacyCode} — ${row.pharmacyName}`);
        else if (filterName === `product`) (row.items || []).filter(item=>!$(`batchFilter`).value||normalize(item.batch)===$(`batchFilter`).value).forEach(item => values.set(normalize(item.productCode || item.productName), `${item.productCode || ``} — ${item.productName}`));
        else if (filterName === `batch`) (row.items || []).filter(item=>!$(`productFilter`).value||normalize(item.productCode||item.productName)===$(`productFilter`).value).forEach(item => values.set(normalize(item.batch), item.batch));
    });
    return [...values.entries()].sort((a,b)=>text(a[1]).localeCompare(text(b[1]),`ar`));
}
function updateSmartFilter(id, filterName, allLabel) {
    const select=$(id), current=select.value, values=smartFilterValues(filterName); select.innerHTML=`<option value="">${allLabel}</option>${values.map(([value,label])=>`<option value="${escapeHtml(value)}">${escapeHtml(label)}</option>`).join(``)}`; select.value=values.some(([value])=>value===current)?current:``; select._searchCombobox?.sync();
}
function refreshSmartFilters() { updateSmartFilter(`statusFilter`,`status`,`جميع الحالات`); updateSmartFilter(`repFilter`,`rep`,`جميع المناديب`); updateSmartFilter(`pharmacyFilter`,`pharmacy`,`جميع الصيدليات`); updateSmartFilter(`productFilter`,`product`,`جميع الأصناف`); updateSmartFilter(`batchFilter`,`batch`,`جميع الـBatches`); }
function renderReviewRows() {
    refreshSmartFilters(); const rows = filteredReviewRows(), body = $(`reviewBody`); $(`countStat`).textContent = rows.length; $(`qtyStat`).textContent = rows.reduce((s,r)=>s+number(r.totalReturnQty),0); $(`paidStat`).textContent = rows.reduce((s,r)=>s+(r.items||[]).reduce((a,x)=>a+number(x.paidReturnQty),0),0); $(`valueStat`).textContent = money(rows.reduce((s,r)=>s+number(r.totalValue),0));
...
function openDrawer(row) {
    const role = document.body.dataset.role; $(`drawer`).classList.add(`show`); document.body.classList.add(`drawer-open`); $(`drawerTitle`).textContent = `مرتجع ${row.pharmacyName}`;
...
function closeDrawer(){ $(`drawer`)?.classList.remove(`show`); document.body.classList.remove(`drawer-open`); }
...
let loginReps = [], selectedSupervisor = ``;
...
async function loginRepresentative(){const rep=loginReps.find(row=>row.id===$(`loginRep`).value),password=text($(`loginRepPassword`).value);if(!rep)return showBanner(`اختر اسم المندوب.`,`error`);if(!password||btoa(password)!==REP_PASSWORDS[text(rep.name)])return showBanner(`كلمة سر المندوب غير صحيحة.`,`error`);if($(`rememberRepLogin`)?.checked)localStorage.setItem(SAVED_REP_LOGIN,JSON.stringify({repId:rep.id,password:btoa(password)}));else localStorage.removeItem(SAVED_REP_LOGIN);writeAccessSession({role:`rep`,repId:rep.id,repName:text(rep.name),supervisorName:repSupervisorMap[text(rep.name)]||``,createdAt:Date.now()});location.href=`rep.html`;}
...
async function initLogin(){document.querySelectorAll(`[data-login-role]`).forEach(button=>button.onclick=()=>showLoginPanel(button.dataset.loginRole));document.querySelectorAll(`.back-roles`).forEach(button=>button.onclick=()=>{$(`roleStep`).classList.remove(`hidden`);$(`repLoginPanel`).classList.add(`hidden`);$(`supervisorLoginPanel`).classList.add(`hidden`);});document.querySelectorAll(`[data-supervisor]`).forEach(button=>button.onclick=()=>{selectedSupervisor=button.dataset.supervisor;document.querySelectorAll(`[data-supervisor]`).forEach(card=>card.classList.toggle(`selected`,card===button));$(`supervisorLoginButton`).disabled=false;});$(`repLoginButton`).onclick=loginRepresentative;$(`supervisorLoginButton`).onclick=loginSupervisor;$(`loginRepPassword`).onkeydown=event=>{if(event.key===`Enter`){event.preventDefault();loginRepresentative();}};$(`loginSupervisorPassword`).onkeydown=event=>{if(event.key===`Enter`){event.preventDefault();loginSupervisor();}};try{const config=await getDoc(doc(db,`system_settings`,`rep_supervisor_assignments`));if(config.exists()&&config.data()?.assignments)repSupervisorMap={...DEFAULT_REP_SUPERVISOR_MAP,...config.data().assignments};}catch(error){console.warn(`تعذر تحميل توزيع المشرفين.`,error);}await loadLoginReps();enhanceCombobox($(`loginRep`),{placeholder:`اكتب اسم المندوب...`});try{const saved=JSON.parse(localStorage.getItem(SAVED_REP_LOGIN)||`null`),rep=loginReps.find(row=>row.id===saved?.repId);if(rep&&saved?.password){$(`loginRep`).value=rep.id;$(`loginRep`)._searchCombobox?.sync();$(`loginRepPassword`).value=atob(saved.password);$(`rememberRepLogin`).checked=true;}}catch(_){localStorage.removeItem(SAVED_REP_LOGIN);}}
