...
async function loadInitialData() {
    try {
        ...
        const CACHE_KEY = 'dad_app_cache_20260622_professional_roadmap1';
        const CACHE_TIME_KEY = 'dad_app_cache_time_20260622_professional_roadmap1';
...
function applyMyOrdersFilters() {
    ...
    filtered.forEach(order => {
        ...
        tr.innerHTML = `
            ...
            <td data-label="السبب">${escapePrintHtml(getOrderRejectionReason(order) || '-')}</td>
            <td data-label="إجراء">
        ...
        unsubReports = onSnapshot(collection(db, "orders"), (snap) => {
            ...
            os.forEach(o => {
                ...
                tr.innerHTML = `
                    ...
                    <td data-label="الحالة"><span class="status-badge ${o.status}">${getWorkflowStatusLabel(o.status)}</span></td>
