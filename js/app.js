...
async function loadInitialData() {
    try {
        ...
        // قراءة البيانات من الكاش إذا لم يمر عليها 12 ساعة
        if (cachedProducts && cacheTime && (now - cacheTime < 43200000)) {
            productsList = JSON.parse(cachedProducts);
            console.log(`تم تحميل ${productsList.length} منتج من الذاكرة المحلية لتوفير القراءات`);
        } else {
            ...
            console.log(`تم تحميل ${productsList.length} منتج من فايربيس`);
        }
...
async function loadManagerOrders() {
    ...
    try {
        ...
        if (managerReps.length > 0) {
            // جلب طلبيات مندوبي هذا المدير فقط بدلاً من جلب كل شيء
            const q = query(collection(db, "orders"), where("repName", "in", managerReps));
        ...
        console.error(e);
        tbody.innerHTML = `<tr><td colspan="8" style="color:red;">خطأ في التحميل</td></tr>`;
    }
}
...
    try {
        ...
        if (!isAdmin && currentRepName) {
            ordersQuery = query(collection(db, "orders"), where("repName", "==", currentRepName));
        }
