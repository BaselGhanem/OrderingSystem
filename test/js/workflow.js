...
function canOrdersStaffTouchOrder(order = {}) {
    const status = getPrimaryStatus(order);
    const staffState = order.orderStaffStatus || '';
    const hidden = status === 'orders_staff_hidden' || staffState === 'orders_staff_hidden' || order.hiddenByOrderStaff === true || order.isInvoiced === true;
    ...
    const renderChunk = async (startIndex = 0) => {
        ...
        chunk.forEach(order => {
            ...
            tr.innerHTML = `
                ...
                <td data-label="الحالة"><span class="status-badge ${escapeHtml(order.status || '')}">${escapeHtml(statusLabel(order.status))}</span></td>
