import { db, collection, getDocs, query, where, addDoc, doc, setDoc, updateDoc, deleteDoc, getDoc, runTransaction } from './firebase.js';
...
async function submitRepReturn() {
    ...
    try {
        ...
        const payload = { repUid: currentAccess.uid, repName, repKey: normalize(repName), supervisorName: text(selectedPharmacy.supervisorName), supervisorKey: normalize(selectedPharmacy.supervisorName), pharmacyCode: selectedPharmacy.pharmacyCode, pharmacyName: selectedPharmacy.pharmacyName, repPharmacyKey: selectedPharmacy.repPharmacyKey, returnType: text($(`returnType`).value), items, totalReturnQty: items.reduce((s, x) => s + x.totalReturnQty, 0), totalValue: items.reduce((s, x) => s + x.lineValue, 0), note: text($(`returnNote`).value), status: `pending_returns_manager`, updatedAt: timestamp };
        if (editingId) {
            await runTransaction(db,async transaction=>{
...
async function loadRepHistory() {
    ...
    try {
        const snap = await getDocs(query(collection(db, RETURNS), where(`repUid`, `==`, currentAccess.uid))); const rows = []; snap.forEach(item => { if (!item.data().deletedAt) rows.push({ id: item.id, ...item.data() }); }); rows.sort((a, b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0));
...
async function loadReviewRows(role) {
    ...
    if (role === `supervisor`) { const key = text(currentAccess?.supervisorKey); if (!key) { location.replace(`login.html`); return; } $(`pageIdentity`).textContent = currentAccess.displayName; snap = await getDocs(query(collection(db, RETURNS), where(`supervisorKey`, `==`, key))); }
...
async function loadInvoiceHistory(row) {
    ...
    try{
        ...
        const [chunks,imports]=await Promise.all([getDocs(chunksQuery),getDocs(collection(db,IMPORTS))]);
...
async function validateHistoricalAllocation(row, items) {
    const allocationQuery = document.body.dataset.role === `supervisor`
        ? query(collection(db, RETURNS), where(`repPharmacyKey`, `==`, row.repPharmacyKey), where(`supervisorKey`, `==`, currentAccess.supervisorKey))
        : query(collection(db, RETURNS), where(`repPharmacyKey`, `==`, row.repPharmacyKey));
    const snap = await getDocs(allocationQuery);
    const used = new Map();
...
async function syncPharmacyDirectory(rows) {
    ...
    rows.forEach(row => {
        ...
        pharmacies.set(key, {
            ...
            pharmacyCode: row.pharmacyCode, pharmacyName: row.pharmacyName, repPharmacyKey: row.repPharmacyKey, updatedAt: now()
        });
    });
