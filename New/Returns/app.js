...
function addLine(prefill = null) {
    ...
    enhanceCombobox(product, { placeholder: `اكتب اسم الصنف أو الكود...` }); enhanceCombobox(batch, { placeholder: `ابحث بالـBatch أو الفاتورة...` });
...
async function submitRepReturn() {
    ...
    try {
        ...
        editingId = ``; $(`returnLines`).innerHTML = ``; $(`returnType`).value = ``; $(`returnType`)._searchCombobox?.sync(); $(`returnNote`).value = ``; await loadPharmacySales(); addLine(); showBanner(`تم إرسال المرتجع لرئيس قسم المرتجعات.`, `success`);
...
async function loadRepHistory() {
    ...
    try {
        ...
        body.innerHTML = rows.length ? rows.map(row => `<tr><td>${dateText(row.createdAt)}</td><td>${escapeHtml(row.pharmacyCode)}<br><strong>${escapeHtml(row.pharmacyName)}</strong></td><td>${row.items.map(item => `${escapeHtml(item.productName)} — ${escapeHtml(item.batch)} — فاتورة ${escapeHtml(item.invoiceNumber)} × ${item.totalReturnQty}`).join(`<br>`)}</td><td>${money(row.totalValue)}</td><td><span class="status ${row.status}">${STATUS_LABELS[row.status] || row.status}</span></td><td>${escapeHtml(row.returnReason || row.rejectionReason || `-`)}</td><td>${row.status === `returned_to_rep` ? `<button class="btn warning edit" data-id="${row.id}">تعديل</button>` : `<button class="btn secondary view-return" data-id="${row.id}"><i class="ph ph-eye"></i> مشاهدة</button>`}</td></tr>`).join(``) : `<tr><td colspan="7"><div class="empty">لا توجد مرتجعات.</div></td></tr>`;
...
function renderReviewShell() {
    ...
    [`searchFilter`,`fromFilter`,`toFilter`].forEach(id => $(id).oninput = () => { syncStatusFilterOptions(); renderReviewRows(); }); $(`statusFilter`).oninput = renderReviewRows; enhanceCombobox($(`statusFilter`), { placeholder: `ابحث عن الحالة...` }); $(`selectAll`).onchange = event => document.querySelectorAll(`.row-check`).forEach(box => box.checked = event.target.checked);
}
