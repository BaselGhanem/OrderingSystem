...
import { loadSystemConfig, guardPage, getAdminSession, normalizeRole, applySystemPresentation } from './system-config.js?v=20260712_settings_control_center_v2';
...
let WORKFLOW_CACHE_VERSION = '20260630_orders_staff_export_columns_finance_note_fix1';
const CACHE_MAX_AGE_MS = 1000 * 60 * 60 * 12;
let PAGE_CACHE_KEY = `dad_orders_${WORKFLOW_CACHE_VERSION}_${WORKFLOW_PAGE || 'workflow'}`;
let ALL_ORDERS_CACHE_KEY = `dad_orders_${WORKFLOW_CACHE_VERSION}_orders_staff_all`;
let workflowSystemConfig = null;

function currentActorName(fallback = '') {
    const session = getAdminSession();
    const role = normalizeRole(session?.role || session?.type || '');
    const user = (workflowSystemConfig?.users || []).find(item => item.id === session?.userId) ||
        (workflowSystemConfig?.users || []).find(item => item.displayName === session?.name && normalizeRole(item.role) === role);
    return user?.displayName || session?.name || fallback;
}
...
function can(permission) {
    ...
    const role = normalizeRole(session?.role || session?.type || '');
    const user = role
        ? (workflowSystemConfig?.users || []).find(item => item.id === session?.userId) ||
          (workflowSystemConfig?.users || []).find(item => item.displayName === session?.name && normalizeRole(item.role) === role)
        : null;
    if (user?.active === false) return false;
    if (!workflowSystemConfig?.general?.enforcePermissions) return true;
    if (!role || !user) return false;
    if (Object.prototype.hasOwnProperty.call(user.permissions || {}, permission)) return user.permissions[permission] === true;
    return workflowSystemConfig.rolePermissions?.[role]?.[permission] === true;
}

function requirePermission(permission) {
    if (can(permission)) return true;
    showToast('لا تملك صلاحية تنفيذ هذا الإجراء.', 'error');
    return false;
}
...
async function saveMarketEdits(options = {}) {
    ...
    await updateOrderWithAudit(state.selectedOrder.id, {
        ...
        marketManagerEditedBy: currentActorName('Market Manager'),
        marketManagerDeletedItems: deleted
    }, auditEntry('market_manager_edit', currentActorName('Market Manager'), 'market_manager', oldValue, { items: kept, grandTotal }, deleted.length ? `Deleted items: ${deleted.length}` : ''));
...
async function marketApprove(orderId, options = {}) {
    ...
    await updateOrderWithAudit(orderId, {
        ...
        marketManagerApprovedBy: currentActorName('Market Manager'),
        marketManagerApprovedAt: new Date(),
        financeStatus: 'finance_pending'
    }, auditEntry('market_manager_approved', currentActorName('Market Manager'), 'market_manager', { status: order.status }, { status: 'finance_pending' }));
...
async function marketReject(orderId, reason = '', options = {}) {
    ...
    await updateOrderWithAudit(orderId, {
        ...
        marketManagerRejectedBy: currentActorName('Market Manager'),
        marketManagerRejectedAt: new Date(),
        marketManagerRejectionReason: reason
    }, auditEntry('market_manager_rejected', currentActorName('Market Manager'), 'market_manager', { status: order.status }, { status: 'market_manager_rejected' }, reason));
...
async function marketDeleteOrder(orderId) {
    ...
    await updateOrderWithAudit(orderId, {
        ...
        deletedByMarketManager: currentActorName('Market Manager'),
        deletedAt: new Date(),
        marketManagerStatus: 'deleted_by_market_manager'
    }, auditEntry('market_manager_order_deleted', currentActorName('Market Manager'), 'market_manager', { status: order.status, items: order.items || [] }, { status: 'deleted_by_market_manager' }, 'Soft delete only'));
...
function handleMarketAction(order, action) {
    ...
    if (action === 'approve') {
        if (!requirePermission('market.approve')) return;
        if (confirm('اعتماد الطلبية وتحويلها إلى المالية؟')) marketApprove(order.id);
        return;
    }
    if (action === 'reject') {
        if (!requirePermission('market.reject')) return;
        const reason = confirmReason('سبب رفض مدير السوق:', true);
        if (reason !== null) marketReject(order.id, reason);
        return;
    }
    if (action === 'return_rep') {
        if (!requirePermission('market.return')) return;
        const reason = confirmRequiredNote('اكتب ملاحظة الإرجاع للمندوب:');
        if (reason !== null) returnOrderStep(order.id, 'representative', reason, currentActorName('Market Manager'), 'market_manager', 'market_manager_returned_to_rep');
        return;
    }
    if (action === 'return_supervisor') {
        if (!requirePermission('market.return')) return;
        const reason = confirmRequiredNote('اكتب ملاحظة الإرجاع للمشرف:');
        if (reason !== null) returnOrderStep(order.id, 'supervisor', reason, currentActorName('Market Manager'), 'market_manager', 'market_manager_returned_to_supervisor');
        return;
    }
    if (action === 'delete') {
        if (!requirePermission('market.return')) return;
...
function handleMarketModalAction(action) {
    ...
    if (action === 'return_rep') {
        if (!requirePermission('market.return')) return;
        const reason = confirmRequiredNote('اكتب ملاحظة الإرجاع للمندوب:');
        if (reason !== null) returnOrderStep(state.selectedOrder.id, 'representative', reason, currentActorName('Market Manager'), 'market_manager', 'market_manager_returned_to_rep').then(closeMarketOrderModal);
        return;
    }
    if (action === 'return_supervisor') {
        if (!requirePermission('market.return')) return;
        const reason = confirmRequiredNote('اكتب ملاحظة الإرجاع للمشرف:');
        if (reason !== null) returnOrderStep(state.selectedOrder.id, 'supervisor', reason, currentActorName('Market Manager'), 'market_manager', 'market_manager_returned_to_supervisor').then(closeMarketOrderModal);
        return;
    }
    if (action === 'delete') {
...
async function marketBulk(action) {
    if (action === 'approve' && !requirePermission('market.approve')) return;
    if (action === 'reject' && !requirePermission('market.reject')) return;
...
function initMarketManager() {
    ...
    if ($('bulkApproveBtn')) $('bulkApproveBtn').hidden = !can('market.approve');
    if ($('bulkRejectBtn')) $('bulkRejectBtn').hidden = !can('market.reject');
    if ($('saveMarketEditsBtn')) $('saveMarketEditsBtn').hidden = !can('market.edit');
    ...
    $('saveMarketEditsBtn')?.addEventListener('click', () => { if (requirePermission('market.edit')) saveMarketEdits(); });
...
async function exportFinanceOrders(orders, scope = 'visible') {
    ...
    const exportEntry = buildExportEntry('finance_excel', currentActorName('Finance Controller'), orders.length, false, financeFileName);
    state.suspendRender = true;
    await Promise.allSettled(orders.map(order => {
        ...
        return updateOrderWithAudit(order.id, {
            ...
            financeExportedBy: currentActorName('Finance Controller')
        }, auditEntry('finance_export', currentActorName('Finance Controller'), 'finance_controller', { status: order.status || '' }, { exportFileName: financeFileName }, 'تصدير مالي بدون تغيير الحالة'));
    }));
    state.suspendRender = false;
    state.onOrdersChange?.();
    ...
    const renderChunk = async (startIndex = 0) => {
        ...
        chunk.forEach(order => {
            ...
            tr.querySelector('.return-market-btn')?.addEventListener('click', () => {
                ...
                if (reason !== null && requirePermission('finance.return')) returnOrderStep(order.id, 'market_manager', reason, currentActorName('Finance Controller'), 'finance_controller', 'finance_returned_to_market_manager');
            });
            fragment.appendChild(tr);
        });
        body.appendChild(fragment);
...
async function financeApprove(orderId, approvalNote = '') {
    if (!requirePermission('finance.approve')) return;
    ...
    await updateOrderWithAudit(orderId, {
        ...
        financeApprovedBy: currentActorName('Finance Controller'),
    ...
    }, auditEntry('finance_approved', currentActorName('Finance Controller'), 'finance_controller', { status: order.status, financeApprovalNote: order.financeApprovalNote || '', financeRejectionReason: order.financeRejectionReason || '' }, { status: 'orders_staff_pending', financeApprovalNote: normalizedNote }, normalizedNote || 'تم الاعتماد المالي بدون ملاحظة إضافية'));
...
async function financeReject(orderId, reason = '') {
    if (!requirePermission('finance.reject')) return;
    const order = state.orders.find(o => o.id === orderId) || {};
    ...
    await updateOrderWithAudit(orderId, {
        ...
        financeRejectedBy: currentActorName('Finance Controller'),
        financeRejectedAt: new Date(),
        financeRejectionReason: normalizedReason,
        financeVisibleNote: normalizedReason,
        financeApprovalNote: ''
    }, auditEntry('finance_rejected', currentActorName('Finance Controller'), 'finance_controller', { status: order.status, financeRejectionReason: order.financeRejectionReason || '' }, { status: 'finance_rejected', financeRejectionReason: normalizedReason }, normalizedReason));
    showToast('تم رفض الطلبية مالياً مع حفظ الملاحظة للمندوب والمشرف.', 'success');
}

function initFinanceController() {
    setDefaultDateFilters();
    if ($('financeExportSelectedBtn')) $('financeExportSelectedBtn').hidden = !can('finance.export');
    if ($('financeExportVisibleBtn')) $('financeExportVisibleBtn').hidden = !can('finance.export');
    bindCommonFilters(applyFinanceFilters);
    $('selectAllWorkflow')?.addEventListener('change', e => document.querySelectorAll('.workflow-order-checkbox').forEach(cb => cb.checked = e.target.checked));
    $('financeExportSelectedBtn')?.addEventListener('click', () => { if (requirePermission('finance.export')) exportFinanceOrders(getOrdersByIds(selectedIds()), 'selected'); });
    $('financeExportVisibleBtn')?.addEventListener('click', () => { if (requirePermission('finance.export')) exportFinanceOrders(state.visibleOrders, 'visible'); });
...
function renderOrdersStaffRows() {
    ...
    const renderChunk = async (startIndex = 0) => {
        ...
        chunk.forEach(order => {
            ...
            const staffActions = [];
            if (staffCanAct && can('staff.return')) staffActions.push(`<button class="action-btn staff-return-btn" type="button"><i class="ph ph-arrow-u-down-left"></i> إرجاع للمالية</button>`);
            if (staffCanAct && can('staff.delete')) staffActions.push(`<button class="action-btn danger-btn staff-delete-btn" type="button"><i class="ph ph-trash"></i> حذف</button>`);
            const staffActionsHtml = staffCanAct
                ? (staffActions.join('') || `<span class="status-badge">عرض فقط</span>`)
...
async function saveStaffEdits(options = {}) {
    ...
    await updateOrderWithAudit(state.selectedOrder.id, {
        ...
        returnedBy: currentActorName('Orders Staff'),
        returnedByRole: 'orders_staff',
        returnedAt: new Date(),
        orderStaffEditedBy: currentActorName('Orders Staff'),
        orderStaffEditedAt: new Date(),
        orderStaffDeletedItems: deleted,
        hiddenByOrderStaff: false
    }, auditEntry('orders_staff_edit_returned_to_finance', currentActorName('Orders Staff'), 'orders_staff', { items: state.selectedOrder.items || [], grandTotal: state.selectedOrder.grandTotal || 0, status: state.selectedOrder.status || '' }, { items: kept, grandTotal, status: 'returned_to_finance' }, reason));
...
async function staffReturnToFinance(orderId, reason) {
    if (!requirePermission('staff.return')) return;
    const order = state.orders.find(o => o.id === orderId) || state.selectedOrder || {};
    if (!canOrdersStaffTouchOrder(order)) {
        showToast('لا يمكن إرجاع طلبية تمت فوترتها أو حذفها.', 'warning');
        return;
    }
    await returnOrderStep(orderId, 'finance', reason, currentActorName('Orders Staff'), 'orders_staff', 'orders_staff_returned_to_finance');
}

async function staffDeleteOrder(orderId) {
    if (!requirePermission('staff.delete')) return;
    const order = state.orders.find(o => o.id === orderId) || state.selectedOrder || {};
    if (!canOrdersStaffTouchOrder(order)) {
    ...
    await updateOrderWithAudit(orderId, {
        ...
        deletedByOrdersStaff: currentActorName('Orders Staff'),
        deletedAt: new Date()
    }, auditEntry('orders_staff_order_deleted', currentActorName('Orders Staff'), 'orders_staff', { status: order.status || '', items: order.items || [] }, { status: 'deleted_by_orders_staff' }, 'حذف ناعم من فريق المعالجة'));
...
async function exportOrders(orders) {
    if (!requirePermission('staff.export')) return;
    if (orders.length === 0) return showToast('لا توجد طلبيات للتصدير.', 'warning');
    if (typeof XLSX === 'undefined') return showToast('مكتبة Excel غير محملة. أعد فتح الصفحة وحاول مرة أخرى.', 'error');
...
إلغاء = لا، أبقِ الطلبيات ظاهرة.`);
    ...
    const results = await Promise.allSettled(orders.map(order => {
        const previousHistory = Array.isArray(order.exportHistory) ? order.exportHistory : [];
        const exportEntry = buildExportEntry('orders_staff_excel', currentActorName('Orders Staff'), orders.length, hide, exportFileName);
        return updateOrderWithAudit(order.id, {
            status: hide ? 'orders_staff_hidden' : 'orders_staff_exported',
            orderStaffStatus: hide ? 'orders_staff_hidden' : 'orders_staff_exported',
            exportedBy: currentActorName('Orders Staff'),
            exportedAt: new Date(),
            exportHistory: [...previousHistory, exportEntry],
            ...
            invoicedBy: hide ? currentActorName('Orders Staff') : ''
        }, auditEntry(action, currentActorName('Orders Staff'), 'orders_staff', { status: order.status, orderStaffStatus: order.orderStaffStatus || '' }, { status: hide ? 'orders_staff_hidden' : 'orders_staff_exported', orderStaffStatus: hide ? 'orders_staff_hidden' : 'orders_staff_exported', exportFileName }));
    }));
    state.suspendRender = false;
    state.onOrdersChange?.();
...
function initOrdersStaff() {
    setDefaultDateFilters();
    ['exportSelectedBtn', 'exportVisibleBtn'].forEach(id => { if ($(id)) $(id).hidden = !can('staff.export'); });
    if ($('saveStaffEditsBtn')) $('saveStaffEditsBtn').hidden = !can('staff.edit');
    if ($('returnStaffToFinanceBtn')) $('returnStaffToFinanceBtn').hidden = !can('staff.return');
    if ($('deleteStaffOrderBtn')) $('deleteStaffOrderBtn').hidden = !can('staff.delete');
...
async function boot() {
    workflowSystemConfig = await loadSystemConfig(true);
    applySystemPresentation(workflowSystemConfig);
    const cacheVersion = workflowSystemConfig?.general?.cacheVersion || WORKFLOW_CACHE_VERSION;
    const revision = workflowSystemConfig?.general?.dataRevision || 1;
    WORKFLOW_CACHE_VERSION = `${cacheVersion}_${revision}`;
    PAGE_CACHE_KEY = `dad_orders_${WORKFLOW_CACHE_VERSION}_${WORKFLOW_PAGE || 'workflow'}`;
    ALL_ORDERS_CACHE_KEY = `dad_orders_${WORKFLOW_CACHE_VERSION}_orders_staff_all`;

    const pagePermission = {
        'market-manager': 'page.market_manager',
        'finance-controller': 'page.finance_controller',
        'orders-staff': 'page.orders_staff'
    }[WORKFLOW_PAGE];
    if (pagePermission && !(await guardPage(pagePermission))) return;
