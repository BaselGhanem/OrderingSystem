...
function ratioHtml(item) { return `<div class="ratio-pair"><span>قبل التعديل <b>${rateText(rateBefore(item))}</b></span><span>بعد التعديل <b>${rateText(bonusRate(item.paidReturnQty,item.bonusReturnQty))}</b></span></div>`; }
...
function withSplit(item, paid, bonus) {
    ...
    return {...item, paidReturnQty:paid, bonusReturnQty:bonus, bonusRateBefore:changed ? (bonusRate(item.paidReturnQty,item.bonusReturnQty) ?? rateBefore(item)) : rateBefore(item), bonusRateAfter:bonusRate(paid,bonus), lineValue:paid*number(item.unitPrice)};
}
...
function auditHtml(entries) {
    ...
    return (entries||[]).slice().reverse().map(entry=>{
        ...
        const changes=(entry.changes||[]).map(change=>`<div class="audit-change"><strong>${escapeHtml(change.productName)}</strong><small>فاتورة ${escapeHtml(change.invoiceNumber)} · ${escapeHtml(change.batch)}</small><div class="audit-comparison"><span>قبل: كمية ${change.before.paid??`—`} / بونص ${change.before.bonus??`—`} / ${rateText(change.before.rate)}</span><b>←</b><span>بعد: كمية ${change.after.paid??`—`} / بونص ${change.after.bonus??`—`} / ${rateText(change.after.rate)}</span></div></div>`).join(``);
...
function openDrawer(row) {
    const role=document.body.dataset.role, representative=role===`representative`, editable=canEditSplit(row,role);
    $(`drawer`).classList.add(`show`);$(`drawerTitle`).textContent=`مرتجع ${row.pharmacyName}`;
    const splitRows=(row.items||[]).map((item,index)=>`<tr data-index="${index}"><td><strong>${escapeHtml(item.productName)}</strong></td><td>${escapeHtml(item.batch)}<br><small>${escapeHtml(item.expiryDate||`—`)}</small></td><td>${escapeHtml(item.invoiceNumber)}</td><td>${item.totalReturnQty}</td><td><input aria-label="الكمية" class="paid-input" type="number" min="0" max="${item.totalReturnQty}" step="1" value="${item.paidReturnQty??``}" ${editable?``:`disabled`}></td><td><input aria-label="البونص" class="bonus-input" type="number" min="0" max="${item.totalReturnQty}" step="1" value="${item.bonusReturnQty??``}" ${editable?``:`disabled`}></td><td class="rate-before">${rateText(rateBefore(item))}</td><td class="rate-after">${rateText(bonusRate(item.paidReturnQty,item.bonusReturnQty))}</td>${representative?``:`<td>${money(item.unitPrice)}</td><td class="line-value">${money(item.lineValue)}</td>`}</tr>`).join(``);
    let actions=``;
    if(editable){actions=role===`returns_manager`?`<button class="btn primary save-split">حفظ وإرسال للمشرف</button><button class="btn warning return-rep">إعادة للمندوب</button><button class="btn danger delete-request">حذف</button>`:role===`supervisor`?`<button class="btn primary save-split">موافقة وإرسال لمدير السوق</button><button class="btn warning return-manager">إعادة لرئيس قسم الطلبيات</button>`:`<button class="btn primary save-split">اعتماد نهائي</button><button class="btn warning return-manager">إعادة لرئيس قسم الطلبيات</button>`;}
    ...
    $(`drawerContent`).querySelectorAll(`.split-table tbody tr`).forEach(tr=>{
        ...
        const sync=source=>{
            ...
            if(input.value===``){other.value=``;tr.querySelector(`.rate-after`).textContent=`غير محددة`;return;}
            ...
            const paid=number(paidInput.value),bonus=number(bonusInput.value), changed=item.paidReturnQty!==paid||item.bonusReturnQty!==bonus;
            tr.querySelector(`.rate-before`).textContent=rateText(changed?(bonusRate(item.paidReturnQty,item.bonusReturnQty)??rateBefore(item)):rateBefore(item));
            tr.querySelector(`.rate-after`).textContent=rateText(bonusRate(paid,bonus));
